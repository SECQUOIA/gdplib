...
def build_model():
    ...
    m.conv_size = Var(
        bounds=(10, 700), initialize=10, doc="Size of conventional plant."
    )
    m.conv_cost = Var()
    ...
    def distance_calculation(m, mkt):
        return (
            ...
            == sqrt(
                ...
                + (m.conv_y / 150 - m.mkt_y[mkt] / 150) ** 2
            )
            * 150
        )
    ...
    def production_satisfaction(m, qtr):
        return m.production[qtr] == sum(
            m.shipments_to_mkt[mkt, qtr] for mkt in m.markets
        )
    ...
    m.variable_shipment_cost = Expression(
        expr=sum(
            m.shipments_to_mkt[mkt, qtr] * m.dist_to_mkt[mkt] * m.transport_cost[qtr]
...
def build_modular_model():
    ...
    m.site_pairs = Set(
        initialize=m.modular_sites * m.modular_sites, filter=lambda _, x, y: not x == y
    )
    m.unique_site_pairs = Set(initialize=m.site_pairs, filter=lambda _, x, y: x < y)
    ...
    m.site_x = Var(m.modular_sites, bounds=(0, 300), initialize={1: 50, 2: 225, 3: 250})
    m.site_y = Var(m.modular_sites, bounds=(0, 300), initialize={1: 300, 2: 275, 3: 50})

    m.num_modules = Var(m.modular_sites, m.quarters, domain=Integers, bounds=(0, 12))
    ...
    m.sqr_scaled_dist_to_site = Var(m.site_pairs, bounds=(0.001, 8), initialize=0.001)
    m.shipments_to_mkt = Var(m.modular_sites, m.markets, m.quarters, bounds=(0, 300))
    ...
    def site_active(disj, site):
        ...
        def module_balance(site_disj, qtr):
            ...
            new_modules = (
                m.modules_added[site, qtr - m.modular_setup_time]
                if qtr > m.modular_setup_time
                else 0
            )
            xfrd_in_modules = sum(
                m.modules_transferred[from_site, site, qtr - m.modular_move_time]
            ...
            xfrd_out_modules = sum(
                ...
                for to_site in m.modular_sites
                if not to_site == site
            )
            return m.num_modules[site, qtr] == (
                new_modules + xfrd_in_modules - xfrd_out_modules + existing_modules
            )
    ...
    def site_inactive(disj, site):
        ...
        disj.no_shipments = Constraint(
            expr=sum(
                m.shipments_to_mkt[site, mkt, qtr]
                for mkt in m.markets
                for qtr in m.quarters
            )
            == 0
    ...
    def site_active_ordering(m, site):
        if site + 1 <= max(m.modular_sites):
            return (
                m.site_active[site].binary_indicator_var
    ...
    def pair_active(disj, site1, site2):
        ...
        disj.site_distance_calc = Constraint(
            expr=m.dist_to_site[site1, site2]
            == sqrt(m.sqr_scaled_dist_to_site[site1, site2]) * 150
        )
    ...
    def product_route_active(disj, site, mkt):
        ...
        def market_distance_calculation(site_disj):
            return (
                m.dist_to_mkt[site, mkt]
                == sqrt(m.sqr_scaled_dist_to_mkt[site, mkt]) * 150
            )
    ...
    def product_route_inactive(disj, site, mkt):
        disj.no_shipments = Constraint(
            expr=sum(m.shipments_to_mkt[site, mkt, qtr] for qtr in m.quarters) == 0
        )
    ...
    m.fixed_shipment_cost = Expression(
        expr=sum(
            m.product_route_active[site, mkt].binary_indicator_var * m.route_fixed_cost
            for site in m.modular_sites
            for mkt in m.markets
        )
    )
    ...
    m.module_purchase_cost = Expression(
        expr=sum(
            m.modules_added[site, qtr] * m.modular_unit_cost[qtr]
            for site in m.modular_sites
            for qtr in m.quarters
        )
    )

    m.module_transfer_cost = Expression(
        expr=sum(
            m.modules_transferred[site1, site2, qtr]
            * m.dist_to_site[site1, site2]
            * m.modular_transport_cost[qtr]
            for site1, site2 in m.site_pairs
            for qtr in m.quarters
        )
    )
...
if __name__ == "__main__":
    ...
    def record_generator():
        for qtr in m.quarters:
            yield (
                (qtr,)
                + tuple(m.num_modules[site, qtr].value for site in m.modular_sites)
                + tuple(m.production[site, qtr].value for site in m.modular_sites)
                + tuple(
                    m.shipments_to_mkt[site, mkt, qtr].value
                    for site in m.modular_sites
                    for mkt in m.markets
                )
                + tuple(m.modules_added[site, qtr].value for site in m.modular_sites)
                + tuple(
                    m.modules_transferred[site1, site2, qtr].value
                    for site1, site2 in m.site_pairs
                )
            )
    ...
    df = pd.DataFrame.from_records(
        ...
        + tuple(
            "ShipSite%stoMkt%s" % (site, mkt)
            for site in m.modular_sites
            for mkt in m.markets
        )
        + tuple("Add Site%s" % site for site in m.modular_sites)
        + tuple("Xfer Site%s to %s" % (site1, site2) for site1, site2 in m.site_pairs),
    )
    ...
    for site1, site2 in m.site_pairs:
        if (
            sum(m.modules_transferred[site1, site2, qtr].value for qtr in m.quarters)
            > 1e-6
        ):
            plt.arrow(
                m.site_x[site1].value,
                m.site_y[site1].value,
