...
def build_model():
    ...
    m.conv_size = Var(
        bounds=(10, 500), initialize=10, doc="Size of conventional plant."
    )
    m.conv_cost = Var()
    ...
    def distance_calculation(m, mkt):
        return (
            ...
            == sqrt(
                ...
                + (m.conv_y / 300 - m.mkt_y[mkt] / 300) ** 2
            )
            * 300
        )
    ...
    def production_satisfaction(m, mo):
        return m.production[mo] == sum(m.shipments_to_mkt[mkt, mo] for mkt in m.markets)
    ...
    m.variable_shipment_cost = Expression(
        expr=sum(
            m.shipments_to_mkt[mkt, mo] * m.dist_to_mkt[mkt] * m.transport_cost[mo]
...
def build_modular_model():
    ...
    m.num_modules = Var(m.modular_sites, m.months, domain=Integers, bounds=(0, 25))
    ...
    m.modules_added = Var(m.modular_sites, m.months, domain=Integers, bounds=(0, 25))
    ...
    m.shipments_to_mkt = Var(m.modular_sites, m.markets, m.months, bounds=(0, 100))
    ...
    def site_active(disj, site):
        ...
        def module_balance(site_disj, mo):
            ...
            new_modules = (
                m.modules_added[site, mo - m.modular_setup_time]
                if mo > m.modular_setup_time
                else 0
            )
            xfrd_in_modules = sum(
                m.modules_transferred[from_site, site, mo - m.modular_move_time]
    ...
    def site_inactive(disj, site):
        ...
        disj.no_shipments = Constraint(
            expr=sum(
                m.shipments_to_mkt[site, mkt, mo]
                for mkt in m.markets
                for mo in m.months
            )
            == 0
    ...
    def product_route_inactive(disj, site, mkt):
        disj.no_shipments = Constraint(
            expr=sum(m.shipments_to_mkt[site, mkt, mo] for mo in m.months) == 0
        )
    ...
    m.module_purchase_cost = Expression(
        expr=sum(
            m.modules_added[site, mo] * m.modular_unit_cost[mo]
            for site in m.modular_sites
            for mo in m.months
        )
    )
...
if __name__ == "__main__":
    ...
    res = SolverFactory('gams').solve(
        m, tee=True, io_options={'add_options': ['option reslim = 300;']}
    )
    ...
    def record_generator():
        for mo in m.months:
            yield (
                (mo,)
                + tuple(m.num_modules[site, mo].value for site in m.modular_sites)
                + tuple(m.production[site, mo].value for site in m.modular_sites)
                + tuple(
                    m.shipments_to_mkt[site, mkt, mo].value
                    for site in m.modular_sites
                    for mkt in m.markets
                )
            )
    ...
    for site1, site2 in m.site_pairs:
        if sum(m.modules_transferred[site1, site2, mo].value for mo in m.months) > 0:
            plt.arrow(
