...
def build_model(use_cafaro_approximation, num_stages):
    ...
    # Map of config index: (# modules, module size)
    ...
    def module_area(m, index):
        ...
        """
        num_modules, size = m.configurations_map[index]
    ...
    def modular_size_cost(m, hot, cold, index):
        """
    ...
    def modular_fixed_cost(m, hot, cold, index):
        """
    ...
    for hot, cold, stg in m.valid_matches * m.stages:
        ...
        disj.choose_one_config = Constraint(
            expr=sum(
                m.module_config_active[hot, cold, stg, index]
                for index in m.module_index_set
            )
            == 1,
        ...
        disj.exchanger_area_cost = Constraint(
            ...
            == sum(
                m.modular_size_cost[hot, cold, index]
                * 1e-3
                * m.module_config_active[hot, cold, stg, index]
                for index in m.module_index_set
            ),
        ...
        disj.exchanger_fixed_cost = Constraint(
            ...
            == sum(
                m.modular_fixed_cost[hot, cold, index]
                * 1e-3
                * m.module_config_active[hot, cold, stg, index]
                for index in m.module_index_set
            ),
        ...
        disj.discretize_area = Constraint(
            ...
            == sum(
                m.module_area[index] * m.module_config_active[hot, cold, stg, index]
                for index in m.module_index_set
            ),
        ...
        disj.discretized_LMTD = Constraint(
            ...
            == sum(
                m.LMTD_discretize[hot, cold, stg, index] for index in m.module_index_set
            ),
            doc="Aggregate LMTD from active configurations for thermal modeling.",
        )

        @disj.Constraint(m.module_index_set)
        def discretized_LMTD_LB(disj, index):
            """
            ...
            return (
                ...
                * m.module_config_active[hot, cold, stg, index]
            ) <= m.LMTD_discretize[hot, cold, stg, index]

        @disj.Constraint(m.module_index_set)
        def discretized_LMTD_UB(disj, index):
            """
            ...
            """
            return m.LMTD_discretize[hot, cold, stg, index] <= (
        ...
        disj.exchanger_required_area = Constraint(
            expr=m.U[hot, cold]
            * sum(
                m.module_area[index] * m.LMTD_discretize[hot, cold, stg, index]
                for index in m.module_index_set
            )
    ...
    def module_type(disj, size):
        ...
        def no_other_module_types(_, hot, cold, stg, index):
            """
            ...
            """
            # num_modules, size = configurations_map[index]
            if m.configurations_map[index][1] != size:
                return m.module_config_active[hot, cold, stg, index] == 0
            else:
