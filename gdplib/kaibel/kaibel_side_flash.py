...
from pyomo.environ import (
    ConcreteModel, Constraint, exp, minimize, NonNegativeReals, Objective, Param, RangeSet, SolverFactory, value, Var, )
...
def calc_side_feed_flash(m):
    ...
    m.xfi = {} # Liquid composition in the side feed of the main model for each component.
    for nc in msf.nc:
        m.xfi[nc] = 1 / m.c

    msf.Tf = Param(doc='Side feed temperature in K',
                   initialize=m.Tf0)
    msf.xf = Var(msf.nc,
    ...
    def _algPvf(msf, nc):
        ...
        return msf.Pvf[nc] == m.prop[nc, 'PC'] * exp(
            m.prop[nc, 'TC'] / msf.Tf * (
                m.prop[nc, 'vpA'] * \
                (1 - msf.Tf / m.prop[nc, 'TC'])
                + m.prop[nc, 'vpB'] * \
                (1 - msf.Tf / m.prop[nc, 'TC'])**1.5
                + m.prop[nc, 'vpC'] * \
                (1 - msf.Tf / m.prop[nc, 'TC'])**3
                + m.prop[nc, 'vpD'] * \
                (1 - msf.Tf / m.prop[nc, 'TC'])**6
            )
        )
    # TODO: Is it computed using the Peng-Robinson equation of state?
