...
def build_model():
    ...
    m.tray_below_so2 = RangeSet(m.sideout2_tray, doc="Trays below side outlet 1")
    ...
    for comp in m.comp:
        ...
        m.hlf[comp] = m.cpdTf[comp, 1]
        m.hvf[comp] = m.cpdTf[comp, 2] + m.dHvap[comp]
    ...
    m.dl = Var(m.dw, doc="Liquid distributor", bounds=(0.2, 0.8), initialize=m.dl0)
    m.dv = Var(
        m.dw,
    ...
    m.rr = Var(
        doc="Internal reflux ratio",
        domain=NonNegativeReals,
        bounds=(0.7, 1),
        initialize=m.rr0,
    )
    m.bu = Var(
        doc="Boilup rate", domain=NonNegativeReals, bounds=(0.7, 1), initialize=m.bu0
    )
    ...
    m.errx = Var(m.section, m.tray, bounds=(-1e-3, 1e-3), initialize=0)
    m.erry = Var(m.section, m.tray, bounds=(-1e-3, 1e-3), initialize=0)
    m.slack = Var(
        m.section,
        m.tray,
        m.comp,
    ...
    m.tray_exists = Disjunct(m.section, m.tray, rule=_build_tray_equations)
    m.tray_absent = Disjunct(m.section, m.tray, rule=_build_pass_through_eqns)
