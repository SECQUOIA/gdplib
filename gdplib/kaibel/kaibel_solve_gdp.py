...
def build_model():
    ...
    m.section_main = Set(initialize=[1, 4],
                         doc="Main sections of the column")
    ...
    m.hllo = (1 - copysign(0.2, hlb[1])) * hlb[1] / m.Hscale # Liquid enthalpy lower bound 
    m.hlup = (1 + copysign(0.2, hlb[2])) * hlb[2] / m.Hscale # Liquid enthalpy upper bound
    m.hvlo = (1 - copysign(0.2, hvb[1])) * hvb[1] / m.Hscale # Vapor enthalpy lower bound
    m.hvup = (1 + copysign(0.2, hvb[2])) * hvb[2] / m.Hscale # Vapor enthalpy upper bound
    ...
    for comp in m.comp:
        m.F0[comp] = m.xfi[comp] * m.Fi # Side feed flowrate per component computed from the feed composition and flowrate Fi
        m.hlf[comp] = (
            m.cpdTf[comp, 1]
    ...
    m.rr = Var(doc="Internal reflux ratio in the column",
               domain=NonNegativeReals,
               bounds=(0.7, 1),
               initialize=m.rr0)
    m.bu = Var(doc="Boilup rate in the reboiler",
               domain=NonNegativeReals,
               bounds=(0.7, 1),
               initialize=m.bu0)
    ...
    #### Global constraints
    @m.Constraint(m.dw, m.tray, doc="Monotonic temperature in the dividing wall sections")
    ...
    m.obj = Objective(
        expr= (m.Qcon + m.Qreb) * m.Hscale + 1e3 * (
            ...
            + 1),
        sense=minimize, doc="Objective function to minimize the operating costs and number of existent trays in the column")





    @m.Constraint(m.section_main, m.candidate_trays_main, doc="Logic proposition for main section")
