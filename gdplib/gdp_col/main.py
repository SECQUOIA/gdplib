...
def display_column(m):
    ...
    print(
        'Qc: {: >3.0f}kW  DB: {: >3.0f} DT: {: >3.0f} dis: {: >3.0f}'.format(
            value(m.Qc * 1e3),
            value(m.D['benzene']),
            value(m.D['toluene']),
            value(m.dis),
    ...
    for t in reversed(list(m.trays)):
        print(
            ...
            'F: {: >4.0f} '
            'L: {: >4.0f} V: {: >4.0f} '
            'xB: {: >3.0f} xT: {: >3.0f} yB: {: >3.0f} yT: {: >3.0f}'.format(
                t,
                fabs(value(m.tray[t].indicator_var)) if t in m.conditional_trays else 1,
                value(m.T[t]) - 273.15,
                value(sum(m.feed[c] for c in m.comps)) if t == m.feed_tray else 0,
                value(m.liq[t]),
                value(m.vap[t]),
    ...
    print(
        'Qb: {: >3.0f}kW  BB: {: > 3.0f} BT: {: >3.0f} bot: {: >3.0f}'.format(
            value(m.Qb * 1e3),
            value(m.B['benzene']),
            value(m.B['toluene']),
            value(m.bot),
    ...
    for t in reversed(list(m.trays)):
        print(
            'T{: >2.0f}-{:1.0g} '
            'FB: {: >3.0f} FT: {: >3.0f} '
            'LB: {: >4.0f} LT: {: >4.0f} VB: {: >4.0f} VT: {: >4.0f}'.format(
                t,
                fabs(value(m.tray[t].indicator_var)) if t in m.conditional_trays else 1,
    ...
    print(
        'RF: {: >3.2f} RB: {: >3.2f}'.format(
            value(m.reflux_frac / (1 - m.reflux_frac)),
