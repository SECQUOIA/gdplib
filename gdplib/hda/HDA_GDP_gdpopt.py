...
def HDA_model():
    ...
    Cppure['tol'] = 225
    Cppure['dip'] = 450
    m.cppure = Param(m.compon, initialize=Cppure, default=0, doc='pure component heat capacities [kj per kg-mol-k]')
    Gcomp = {}
    Gcomp[7, 'h2'] = 0.95
    Gcomp[7, 'ch4'] = 0.05
    Gcomp[8, 'h2'] = 0.5
    ...
    m.perm = Param(m.compon, initialize=Permset, default=0, doc='permeability [kg-mole/m**2-min-mpa]')
    ...
    # distillation
    m.nmin = Var(m.dist, within=NonNegativeReals, initialize=1, doc='minimum number of trays in column')
    ...
    m.distp = Var(m.dist, within=NonNegativeReals, initialize=1, bounds=(0.1, 4.0), doc='column pressure [mega-pascal]')
    ...
    # furnace
    m.qfuel = Var(m.furn, within=NonNegativeReals, bounds=(None, 10), initialize=1, doc='heating required [1.e+12 kj per yr]')
    # cooler
    ...
    m.rctp = Var(m.rct, within=NonNegativeReals, bounds=(3.4474, 3.4474), doc='reactor pressure [mega-pascal]')
    ...
    m.spl1p = Var(m.spl1, within=PositiveReals, bounds=(0.1, 4.0), doc='splitter pressure [mega-pascal]')
    # splitter
    m.splp = Var(m.spl, within=Reals, bounds=(0.1, 4.0), doc='splitter pressure [mega-pascal]')
    ...
    m.p = Var(m.str, within=NonNegativeReals, bounds=(0.1, 4.0), initialize=3.0, doc='stream pressure [mega-pascal]')
    ...
    for abso in m.abs:
        for compon in m.compon:
            m.beta[abso, compon].setlb(log((1 - m.aabs[compon] ** (m.nabs[1].lb * m.abseff + m.eps1 + 1)) / (1 - m.aabs[compon])))
            m.beta[abso, compon].setub(min(15, log((1 - m.aabs[compon] ** (m.nabs[1].ub * m.abseff + m.eps1 + 1)) / (1 - m.aabs[compon]))))
    m.t[67].setlb(3.0)
    m.t[67].setub(3.0)
    for compon in m.compon:
        ...
        for stream in m.str:
            for compon in m.compon:
                ...
                if (dist, stream) in m.ldist and (dist, compon) in m.dhkey:
                    m.avevlt[dist].setlb(
                        m.avevlt[dist].lb/m.vp[stream, compon].ub)
    for dist in m.dist:
        for stream in m.str:
            for compon in m.compon:
                ...
                if (dist, stream) in m.vdist and (dist, compon) in m.dhkey:
                    m.avevlt[dist].setub(
                        m.avevlt[dist].ub/m.vp[stream, compon].lb)
    ...
    for i in range(len(streamfc)):
        m.fc[streamfc.to_numpy()[i, 0], comp.to_numpy()[
            i, 0]] = fc.to_numpy()[i, 0]
        m.vp[streamvp.to_numpy()[i, 0], compvp.to_numpy()[
            i, 0]] = vp.to_numpy()[i, 0]
    ...
    for i in range(len(stream4)):
        m.eflsh[stream4.to_numpy()[i, 0], comp2.to_numpy()[
            i, 0]] = eflsh.to_numpy()[i, 0]
    for i in range(6):
    ...
    def build_absorber(b, absorber):
        ...
        b.absfact = Constraint([absorber], m.compon, rule=Absfact, doc='absorption factor equation')
        ...
        def Betaeqn(_m, i, compon):
            ...
            if (i, compon) not in m.asimp:
                return m.beta[i, compon] == log((1 - m.aabs[compon] ** (m.nabs[i] * m.abseff + 1)) / (1 - m.aabs[compon]))
            return Constraint.Skip
        b.betaeqn = Constraint([absorber], m.compon,
                               rule=Betaeqn, doc='definition of beta')

        def Abssvrec(_m, i, compon):
            # recovery of solvent
            if (i, compon) in m.asolv:
                return sum(m.fc[stream, compon] for (i, stream) in m.ovabs) * exp(m.beta[i, compon]) == sum(m.fc[stream, compon] for (i_, stream) in m.ivabs) + exp(m.gamma[i, compon]) * sum(m.fc[stream, compon] for (i_, stream) in m.ilabs)
            return Constraint.Skip
        b.abssvrec = Constraint([absorber], m.compon,
                                rule=Abssvrec, doc='recovery of solvent')

        def Absrec(_m, i, compon):
            # recovery of non-solvent
            if (i, compon) in m.anorm:
                return sum(m.fc[i, compon] for (abs, i) in m.ovabs) * exp(m.beta[i, compon]) == sum(m.fc[i, compon] for(abs, i) in m.ivabs)
            return Constraint.Skip
        b.absrec = Constraint([absorber], m.compon,
                              rule=Absrec, doc='recovery of non-solvent')

        def abssimp(_m, absorb, compon):
            # recovery of simplified components
            if (absorb, compon) in m.asimp:
                return sum(m.fc[i, compon] for (absorb, i) in m.ovabs) == sum(m.fc[i, compon] for (absorb, i) in m.ivabs) / m.cbeta[compon]
            return Constraint.Skip
    ...
    m.vapor_pressure_unit_match = Param(
        initialize=7500.6168, doc="unit match coefficient for vapor pressure calculation")
    m.actual_reflux_ratio = Param(
        initialize=1.2, doc="actual reflux ratio coefficient")
    ...
    def build_distillation(b, dist):
        ...
        def Antdistb(_m, dist_, stream, compon):
            if (dist_, stream) in m.ldist and (dist_, compon) in m.dkey and dist_ == dist:
                return log(m.vp[stream, compon] * m.vapor_pressure_unit_match) == m.anta[compon] - m.antb[compon] / (m.t[stream] * 100. + m.antc[compon])
            return Constraint.Skip
        b.antdistb = Constraint(
            [dist], m.str, m.compon, rule=Antdistb, doc='vapor pressure correlation (bot)')

        def Antdistt(_m, dist_, stream, compon):
            if (dist_, stream) in m.vdist and (dist_, compon) in m.dkey and dist == dist_:
                return log(m.vp[stream, compon] * m.vapor_pressure_unit_match) == m.anta[compon] - m.antb[compon] / (m.t[stream] * 100. + m.antc[compon])
            return Constraint.Skip
        b.antdistt = Constraint(
            [dist], m.str, m.compon, rule=Antdistt, doc='vapor pressure correlation (top)')

        def Relvol(_m, dist_):
            if dist == dist_:
                divided1 = sum(sum(m.vp[stream, compon] for (dist_, compon) in m.dlkey if dist_ == dist)/sum(m.vp[stream, compon]
                                                                                                             for (dist_, compon) in m.dhkey if dist_ == dist) for (dist_, stream) in m.vdist if dist_ == dist)
                divided2 = sum(sum(m.vp[stream, compon] for (dist_, compon) in m.dlkey if dist_ == dist)/sum(m.vp[stream, compon]
                                                                                                             for (dist_, compon) in m.dhkey if dist_ == dist) for (dist_, stream) in m.ldist if dist_ == dist)
                return m.avevlt[dist] == sqrt(divided1 * divided2)
            return Constraint.Skip
        b.relvol = Constraint([dist], rule=Relvol,
                              doc='average relative volatility')

        def Undwood(_m, dist_):
            # minimum reflux ratio from Underwood equation
            if dist_ == dist:
                return sum(m.fc[stream, compon] for (dist1, compon) in m.dlkey if dist1 == dist_ for (dist1, stream) in m.idist if dist1 == dist_) * m.rmin[dist_] * (m.avevlt[dist_] - 1) == sum(m.f[stream] for (dist1, stream) in m.idist if dist1 == dist_)
            return Constraint.Skip
        b.undwood = Constraint([dist], rule=Undwood,
                               doc='minimum reflux ratio equation')
        ...
        def Actreflux(_m, dist_):
            ...
            return Constraint.Skip
        b.actreflux = Constraint(
            [dist], rule=Actreflux, doc='actual reflux ratio')

        def Fenske(_m, dist_):
            # minimum number of trays from Fenske equation
            if dist == dist_:
                sum1 = sum((m.f[stream] + m.eps1)/(m.fc[stream, compon] + m.eps1) for (dist1, compon)
                           in m.dhkey if dist1 == dist_ for (dist1, stream) in m.vdist if dist1 == dist_)
                sum2 = sum((m.f[stream] + m.eps1)/(m.fc[stream, compon] + m.eps1) for (dist1, compon)
                           in m.dlkey if dist1 == dist_ for (dist1, stream) in m.ldist if dist1 == dist_)
                return m.nmin[dist_] * log(m.avevlt[dist_]) == log(sum1 * sum2)
            return Constraint.Skip
        b.fenske = Constraint([dist], rule=Fenske,
                              doc='minimum number of trays')

        def Acttray(_m, dist_):
            # actual number of trays (gillilands approximation)
            if dist == dist_:
                return m.ndist[dist_] == m.nmin[dist_] * 2. / m.disteff
            return Constraint.Skip
        b.acttray = Constraint([dist], rule=Acttray,
                               doc='actual number of trays')

        def Distspec(_m, dist_, stream, compon):
            if (dist_, stream) in m.vdist and (dist_, compon) in m.dhkey and dist_ == dist:
                return m.fc[stream, compon] <= m.recovery_specification_coefficient * sum(m.fc[str2, compon] for (dist_, str2) in m.idist if dist == dist_)
            return Constraint.Skip
        b.distspec = Constraint([dist], m.str, m.compon,
                                rule=Distspec, doc='recovery specification')

        def Distheav(_m, dist_, compon):
            if (dist_, compon) in m.dh and dist == dist_:
                return sum(m.fc[str2, compon] for (dist_, str2) in m.idist if dist_ == dist) == sum(m.fc[str2, compon] for (dist_, str2) in m.ldist if dist_ == dist)
            return Constraint.Skip
        b.distheav = Constraint(
            [dist], m.compon, rule=Distheav, doc='heavy components')

        def Distlite(_m, dist_, compon):
            if (dist_, compon) in m.dl and dist_ == dist:
                return sum(m.fc[str2, compon] for (dist_, str2) in m.idist if dist == dist_) == sum(m.fc[str2, compon] for (dist_, str2) in m.vdist if dist == dist_)
            return Constraint.Skip
        b.distlite = Constraint(
            [dist], m.compon, rule=Distlite, doc='light components')

        def Distpi(_m, dist_, stream):
            if (dist_, stream) in m.idist and dist_ == dist:
                return m.distp[dist_] <= m.p[stream]
            return Constraint.Skip
        b.distpi = Constraint([dist], m.str, rule=Distpi,
                              doc='inlet pressure relation')

        def Distvpl(_m, dist_, stream):
            if (dist_, stream) in m.ldist and dist == dist_:
                return m.distp[dist_] == sum(m.vp[stream, compon] for (dist_, compon) in m.dhkey if dist_ == dist)
            return Constraint.Skip
        b.distvpl = Constraint([dist], m.str, rule=Distvpl,
                               doc='bottom vapor pressure relation')

        def Distvpv(_m, dist_, stream):
            if dist > 1 and (dist, stream) in m.vdist and dist_ == dist:
                return m.distp[dist_] == sum(m.vp[stream, compon] for (dist_, compon) in m.dlkey if dist_ == dist)
            return Constraint.Skip
        b.distvpv = Constraint([dist], m.str, rule=Distvpv,
                               doc='top vapor pressure relation')

        def Distpl(_m, dist_, stream):
            if (dist_, stream) in m.ldist and dist_ == dist:
                return m.distp[dist_] == m.p[stream]
            return Constraint.Skip
        b.distpl = Constraint([dist], m.str, rule=Distpl,
                              doc='outlet pressure relation (liquid)')

        def Distpv(_m, dist_, stream):
            if (dist_, stream) in m.vdist and dist == dist_:
                return m.distp[dist_] == m.p[stream]
            return Constraint.Skip
        b.distpv = Constraint([dist], m.str, rule=Distpv,
                              doc='outlet pressure relation (vapor)')
    ...
    def build_flash(b, flsh):
        ...
        def Antflsh(_m, flsh_, stream, compon):
            if (flsh_, stream) in m.lflsh and flsh_ == flsh:
                return log(m.vp[stream, compon] * m.vapor_pressure_unit_match) == m.anta[compon] - m.antb[compon] / (m.t[stream] * 100. + m.antc[compon])
            return Constraint.Skip
        b.antflsh = Constraint([flsh], m.str, m.compon,
                               rule=Antflsh, doc='flash pressure relation')

        def Flshrec(_m, flsh_, stream, compon):
            if (flsh_, stream) in m.lflsh and flsh_ == flsh:
                return sum(m.eflsh[flsh1, compon2] for (flsh1, compon2) in m.fkey if flsh1 == flsh_) * (m.eflsh[flsh_, compon] * sum(m.vp[stream, compon2] for (flsh1, compon2) in m.fkey if flsh_ == flsh1) + (1. - m.eflsh[flsh_, compon]) * m.vp[stream, compon]) == sum(m.vp[stream, compon2] for (flsh1, compon2) in m.fkey if flsh_ == flsh1) * m.eflsh[flsh_, compon]
            return Constraint.Skip
        b.flshrec = Constraint([flsh], m.str, m.compon,
                               rule=Flshrec, doc='vapor recovery relation')

        def Flsheql(_m, flsh_, compon):
            if flsh in m.flsh and compon in m.compon and flsh_ == flsh:
                return sum(m.fc[stream, compon] for (flsh1, stream) in m.vflsh if flsh1 == flsh_) == sum(m.fc[stream, compon] for (flsh1, stream) in m.iflsh if flsh1 == flsh_) * m.eflsh[flsh, compon]
            return Constraint.Skip
        ...
        def Flshpr(_m, flsh_, stream):
            if (flsh_, stream) in m.lflsh and flsh_ == flsh:
                return m.flshp[flsh_] * m.f[stream] == sum(m.vp[stream, compon] * m.fc[stream, compon] for compon in m.compon)
            return Constraint.Skip
        b.flshpr = Constraint([flsh], m.str, rule=Flshpr,
                              doc='flash pressure relation')

        def Flshpi(_m, flsh_, stream):
            if (flsh_, stream) in m.iflsh and flsh_ == flsh:
                return m.flshp[flsh_] == m.p[stream]
            return Constraint.Skip
        b.flshpi = Constraint([flsh], m.str, rule=Flshpi,
                              doc='inlet pressure relation')
        ...
        def Flshpl(_m, flsh_, stream):
            if (flsh_, stream) in m.lflsh and flsh_ == flsh:
                return m.flshp[flsh_] == m.p[stream]
            return Constraint.Skip
        b.flshpl = Constraint([flsh], m.str, rule=Flshpl,
                              doc='outlet pressure relation (liquid)')
        ...
        def Flshpv(_m, flsh_, stream):
            if (flsh_, stream) in m.vflsh and flsh_ == flsh:
                return m.flshp[flsh_] == m.p[stream]
            return Constraint.Skip
        b.flshpv = Constraint([flsh], m.str, rule=Flshpv,
                              doc='outlet pressure relation (vapor)')
        ...
        def Flshti(_m, flsh_, stream):
            ...
            return Constraint.Skip
        b.flshti = Constraint([flsh], m.str, rule=Flshti,
                              doc='inlet temp. relation')

        def Flshtl(_m, flsh_, stream):
            if (flsh_, stream) in m.lflsh and flsh_ == flsh:
                return m.flsht[flsh_] == m.t[stream]
            return Constraint.Skip
        b.flshtl = Constraint([flsh], m.str, rule=Flshtl,
                              doc='outlet temp. relation (liquid)')

        def Flshtv(_m, flsh_, stream):
            if (flsh_, stream) in m.vflsh and flsh_ == flsh:
                return m.flsht[flsh_] == m.t[stream]
            return Constraint.Skip
        b.flshtv = Constraint([flsh], m.str, rule=Flshtv,
                              doc='outlet temp. relation (vapor)')

    
    m.heat_unit_match = Param(
        initialize=3600. * 8500. * 1.0e-12 / 60., doc="unit change on temp")
    ...
    def build_heater(b, heater):
        ...
        def Hehcmb(_m, heh, compon):
            if heh == heater and compon in m.compon:
                return sum(m.fc[stream, compon] for (heh_, stream) in m.oheh if heh_ == heh) == sum(m.fc[stream, compon] for (heh_, stream) in m.iheh if heh == heh_)
            return Constraint.Skip
        b.hehcmb = Constraint(Set(
            initialize=[heater]), m.compon, rule=Hehcmb, doc='component balance in heater')

        def Hehhb(_m, heh):
            if heh == heater:
                return m.qh[heh] == (sum(m.cp[stream] * m.f[stream] * 100. * m.t[stream] for (heh_, stream) in m.oheh if heh_ == heh) -
        ...
        def hehp(_m, heh):
            if heh == heater:
                return sum(m.p[stream] for(heh_, stream) in m.iheh if heh_ == heh) == sum(m.p[stream] for(heh_, stream) in m.oheh if heh == heh_)
            return Constraint.Skip
        b.Hehp = Constraint(
    ...
    def build_exchanger(b, exchanger):
        ...
        b.exchdtm1 = Constraint(
            [exchanger], rule=Exchdtm1, doc='delta t min condition')

        def Exchdtm2(_m, exch):
            return sum(m.t[stream] for (exch, stream) in m.ocexch) <= sum(m.t[stream] for (exch, stream) in m.ihexch) - m.exchanger_temp_drop
        b.exchdtm2 = Constraint(
            [exchanger], rule=Exchdtm2, doc='delta t min condition')
    ...
    def build_membrane(b, membrane):
        ...
        def Memtp(_m, memb, stream):
            if (memb, stream) in m.pmemb and memb == membrane:
                return m.t[stream] == sum(m.t[stream2] for (memb, stream2) in m.imemb if memb == membrane)
            return Constraint.Skip
        b.memtp = Constraint([membrane], m.str, rule=Memtp,
                             doc='temp relation for permeate')

        def Mempp(_m, memb, stream):
            if (memb, stream) in m.pmemb and memb == membrane:
                return m.p[stream] <= sum(m.p[stream2] for (memb, stream2) in m.imemb if memb == membrane)
            return Constraint.Skip
        b.mempp = Constraint([membrane], m.str, rule=Mempp,
                             doc='pressure relation for permeate')

        def Memtn(_m, memb, stream):
            if (memb, stream) in m.nmemb and memb == membrane:
                return m.t[stream] == sum(m.t[stream2] for (memb, stream2) in m.imemb if memb == membrane)
            return Constraint.Skip
        b.Memtn = Constraint([membrane], m.str, rule=Memtn,
                             doc='temp relation for non-permeate')

        def Mempn(_m, memb, stream):
            if (memb, stream) in m.nmemb and memb == membrane:
                return m.p[stream] == sum(m.p[stream] for (memb_, stream) in m.imemb if memb_ == memb)
            return Constraint.Skip
        b.Mempn = Constraint([membrane], m.str, rule=Mempn,
                             doc='pressure relation for non-permeate')

        def Rec(_m, memb_, stream):
            if (memb_, stream) in m.pmemb and memb_ == membrane:
                return m.fc[stream, 'h2'] >= m.membrane_recovery_sepc * sum(m.fc[stream, 'h2'] for (memb, stream) in m.imemb if memb == memb_)
            return Constraint.Skip
    ...
    def build_multiple_mixer(b, multiple_mxr):
        ...
        def Mxrcmb(_b, mxr, compon):
            if mxr == multiple_mxr:
                return sum(m.fc[stream, compon] for (mxr_, stream) in m.omxr if mxr == mxr_) == sum(m.fc[stream, compon] for (mxr_, stream) in m.imxr if mxr == mxr_)
            return Constraint.Skip
        b.mxrcmb = Constraint([multiple_mxr], m.compon,
                              rule=Mxrcmb, doc='component balance in mixer')

        def Mxrhb(_b, mxr):
            if mxr == multiple_mxr and mxr != 2:
                return sum(m.f[stream] * m.t[stream] * m.cp[stream] for (mxr_, stream) in m.imxr if mxr == mxr_) == sum(m.f[stream] * m.t[stream] * m.cp[stream] for (mxr_, stream) in m.omxr if mxr == mxr_)
            return Constraint.Skip
        b.mxrhb = Constraint([multiple_mxr], rule=Mxrhb,
                             doc="heat balance in mixer")

        def Mxrhbq(_b, mxr):
            if mxr == 2 and mxr == multiple_mxr:
                return m.f[16] * m.t[16] == m.f[15] * m.t[15] - (m.fc[20, 'ben'] + m.fc[20, 'tol']) * m.heatvap['tol'] / (100. * m.cp[15])
            return Constraint.Skip
        b.mxrhbq = Constraint([multiple_mxr], rule=Mxrhbq,
                              doc='heat balance in quench')

        def Mxrpi(_b, mxr, stream):
            if (mxr, stream) in m.imxr and mxr == multiple_mxr:
                return m.mxrp[mxr] == m.p[stream]
            return Constraint.Skip
        b.mxrpi = Constraint([multiple_mxr], m.str,
                             rule=Mxrpi, doc='inlet pressure relation')

        def Mxrpo(_b, mxr, stream):
            if (mxr, stream) in m.omxr and mxr == multiple_mxr:
                return m.mxrp[mxr] == m.p[stream]
            return Constraint.Skip
        b.mxrpo = Constraint([multiple_mxr], m.str,
                             rule=Mxrpo, doc='outlet pressure relation')
    ...
    def build_multiple_splitter(b, multi_splitter):
        ...
        def Splpi(_m, spl, stream):
            ...
            return Constraint.Skip
        b.splpi = Constraint([multi_splitter], m.str,
                             rule=Splpi, doc='inlet pressure relation (splitter)')

        def Splpo(_m, spl, stream):
            if (spl, stream) in m.ospl and spl == multi_splitter:
                return m.splp[spl] == m.p[stream]
            return Constraint.Skip
        b.splpo = Constraint([multi_splitter], m.str,
                             rule=Splpo, doc='outlet pressure relation (splitter)')

        def Splti(_m, spl, stream):
            if (spl, stream) in m.ispl and spl == multi_splitter:
                return m.splt[spl] == m.t[stream]
            return Constraint.Skip
        b.splti = Constraint([multi_splitter], m.str,
                             rule=Splti, doc='inlet temperature relation (splitter)')
        ...
        b.splto = Constraint([multi_splitter], m.str,
                             rule=Splto, doc='outlet temperature relation (splitter)')
    ...
    m.Ea_R = Param(initialize=-26167.,doc="Activation energy for reaction rate constant")
    ...
    def build_reactor(b, rct):
        ...
        def rctspec(_m, rct, stream):
            if (rct, stream) in m.irct:
                return m.fc[stream, 'h2'] >= 5 * (m.fc[stream, 'ben'] + m.fc[stream, 'tol'] + m.fc[stream, 'dip'])
            return Constraint.Skip
        b.Rctspec = Constraint([rct], m.str, rule=rctspec,
                               doc='spec. on reactor feed stream')

        def rxnrate(_m, rct):
            return m.krct[rct] == m.Prereference_factor * exp(m.Ea_R / (m.rctt[rct] * 100.))
        b.Rxnrate = Constraint([rct], rule=rxnrate,
                               doc='reaction rate constant')

        def rctconv(_m, rct, stream, compon):
            if (rct, compon) in m.rkey and (rct, stream) in m.irct:
                return 1. - m.conv[rct, compon] == (1. / (1. + m.conversion_coefficient * m.krct[rct] * m.rctvol[rct] * sqrt(m.fc[stream, compon] / 60 + m.eps1) * (m.f[stream] / 60. + m.eps1) ** (-3./2.))) ** 2.
            return Constraint.Skip
        b.Rctconv = Constraint([rct], m.str, m.compon,
                               rule=rctconv, doc="conversion of key component")

        def rctsel(_m, rct):
            return (1. - m.sel[rct]) == m.selectivity_1 * (1. - m.conv[rct, 'tol']) ** m.selectivity_2
        b.Rctsel = Constraint([rct], rule=rctsel,
                              doc='selectivity to benzene')

        def rctcns(_m, rct, stream, compon):
            if (rct, compon) in m.rkey and (rct, stream) in m.irct:
                return m.consum[rct, compon] == m.conv[rct, compon] * m.fc[stream, compon]
            return Constraint.Skip
        b.Rctcns = Constraint([rct], m.str, m.compon,
                              rule=rctcns, doc='consumption rate of key comp.')

        def rctmbtol(_m, rct):
            return sum(m.fc[stream, 'tol'] for (rct_, stream) in m.orct if rct_ == rct) == sum(m.fc[stream, 'tol'] for (rct_, stream) in m.irct if rct_ == rct) - m.consum[rct, 'tol']
        b.Rctmbtol = Constraint([rct], rule=rctmbtol,
                                doc='mass balance in reactor (tol)')

        def rctmbben(_m, rct):
            return sum(m.fc[stream, 'ben'] for (rct_, stream) in m.orct if rct_ == rct) == sum(m.fc[stream, 'ben'] for (rct_, stream) in m.irct if rct_ == rct) + m.consum[rct, 'tol'] * m.sel[rct]
        b.Rctmbben = Constraint([rct], rule=rctmbben, doc='mass balance in reactor (ben)')

        def rctmbdip(_m, rct):
            return sum(m.fc[stream, 'dip'] for (rct1, stream) in m.orct if rct1 == rct) == sum(m.fc[stream, 'dip'] for (rct1, stream) in m.irct if rct1 == rct) + m.consum[rct, 'tol'] * 0.5 + (sum(m.fc[stream, 'ben'] for (rct1, stream) in m.irct if rct1 == rct) - sum(m.fc[stream, 'ben'] for (rct1, stream) in m.orct if rct1 == rct)) * 0.5
        b.Rctmbdip = Constraint([rct], rule=rctmbdip, doc='mass balance in reactor (dip)')

        def rctmbh2(_m, rct):
            return sum(m.fc[stream, 'h2'] for (rct1, stream) in m.orct if rct1 == rct) == sum(m.fc[stream, 'h2'] for (rct1, stream) in m.irct if rct1 == rct) - m.consum[rct, 'tol'] - sum(m.fc[stream, 'dip'] for (rct1, stream) in m.irct if rct1 == rct) + sum(m.fc[stream, 'dip'] for (rct1, stream) in m.orct if rct1 == rct)
        ...
        def rctpi(_m, rct, stream):
            ...
            return Constraint.Skip
        b.Rctpi = Constraint([rct], m.str, rule=rctpi,
                             doc='inlet pressure relation')
        ...
        def rctpo(_m, rct, stream):
            ...
            return Constraint.Skip
        b.Rctpo = Constraint([rct], m.str, rule=rctpo,
                             doc='outlet pressure relation')

        def rcttave(_m, rct):
            return m.rctt[rct] == (sum(m.t[stream] for (rct1, stream) in m.irct if rct1 == rct) + sum(m.t[stream] for (rct1, stream) in m.orct if rct1 == rct))/2
        b.Rcttave = Constraint([rct], rule=rcttave,
                               doc='average temperature relation')

        def Rctmbch4(_m, rct):
            return sum(m.fc[stream, 'ch4'] for (rct_, stream) in m.orct if rct_ == rct) == sum(m.fc[stream, 'ch4'] for (rct_, stream) in m.irct if rct == rct_) + m.consum[rct, 'tol']
        b.rctmbch4 = Constraint([rct], rule=Rctmbch4,
                                doc='mass balance in reactor (ch4)')

        def Rcthbadb(_m, rct):
            if rct == 1:
                return m.heatrxn[rct] * m.consum[rct, 'tol'] / 100. == sum(m.cp[stream] * m.f[stream] * m.t[stream] for (rct_, stream) in m.orct if rct_ == rct) - sum(m.cp[stream] * m.f[stream] * m.t[stream] for (rct_, stream) in m.irct if rct_ == rct)
            return Constraint.Skip
        b.rcthbadb = Constraint([rct], rule=Rcthbadb,
                                doc='heat balance (adiabatic)')

        def Rcthbiso(_m, rct):
            if rct == 2:
                return m.heatrxn[rct] * m.consum[rct, 'tol'] * 60. * 8500 * 1.0e-09 == m.q[rct]
            return Constraint.Skip
        b.rcthbiso = Constraint([rct], rule=Rcthbiso,
                                doc='temp relation (isothermal)')

        def Rctisot(_m, rct):
            if rct == 2:
                return sum(m.t[stream] for (rct_, stream) in m.irct if rct_ == rct) == sum(m.t[stream] for (rct_, stream) in m.orct if rct_ == rct)
            return Constraint.Skip
        b.rctisot = Constraint([rct], rule=Rctisot,
                               doc='temp relation (isothermal)')
    ...
    def build_single_mixer(b, mixer):
        ...
        def Mxr1cmb(m_, mxr1, str1, compon):
            if (mxr1, str1) in m.omxr1 and mxr1 == mixer:
                return m.fc[str1, compon] == sum(m.fc[str2, compon] for (mxr1_, str2) in m.imxr1 if mxr1_ == mxr1)
            return Constraint.Skip
        b.mxr1cmb = Constraint([mixer], m.str, m.compon,
                               rule=Mxr1cmb, doc='component balance in mixer')
    ...
    def build_single_splitter(b, splitter):
        ...
        def Spl1cmb(m_, spl1, compon):
            return sum(m.fc[str1, compon] for (spl1_, str1) in m.ospl1 if spl1_ == spl1) == sum(m.fc[str1, compon] for (spl1_, str1) in m.ispl1 if spl1_ == spl1)
        ...
        def Spl1pi(m_, spl1, str1):
            ...
            return Constraint.Skip
        b.spl1pi = Constraint([splitter], m.str, rule=Spl1pi,
        ...
        def Spl1po(m_, spl1, str1):
            if (spl1, str1) in m.ospl1:
                return m.spl1p[spl1] == m.p[str1]
            return Constraint.Skip
        b.spl1po = Constraint([splitter], m.str, rule=Spl1po,
        ...
        def Spl1ti(m_, spl1, str1):
            ...
            return Constraint.Skip
        b.spl1ti = Constraint([splitter], m.str, rule=Spl1ti,
        ...
        def Spl1to(m_, spl1, str1):
            if (spl1, str1) in m.ospl1:
                return m.spl1t[spl1] == m.t[str1]
            return Constraint.Skip
        b.spl1to = Constraint([splitter], m.str, rule=Spl1to,
    ...
    m.hydrogen_purge_value = Param(initialize = 1.08,doc = "heating value of hydrogen purge")
    m.electricity_cost = Param(initialize = 0.04 * 24 * 365 / 1000 , doc ="electricity cost, value is 0.04 with the unit of [kw/h], now is [kw/yr/k$]")
    m.meathane_purge_value = Param(initialize = 3.37, doc = "heating value of meathane purge")
    m.heating_cost = Param(initialize = 8000., doc = "Heating cost (steam) with unit [1e6 kj]") 
    m.cooling_cost = Param(initialize = 700.0, doc = "heating cost (water) with unit [1e6 kj]")
    ...
    m.abs_linear_coeffcient = Param(initialize = 1.2, doc = "linear coeffcient of absorber (times tray number) [$1e3 per year]")
    m.compressor_fixed_cost = Param(initialize = 7.155, doc = "compressor fixed cost [$1e3 per year]")
    m.compressor_fixed_cost_4 = Param(initialize = 4.866, doc = "compressor fixed cost for compressor 4 [$1e3 per year]")
    m.compressor_linear_coeffcient = Param(initialize = 0.815 ,doc = "compressor linear coeffcient (vaporflow rate) [$1e3 per year]")
    m.compressor_linear_coeffcient_4 = Param(initialize = 0.887 ,doc = "compressor linear coeffcient (vaporflow rate) [$1e3 per year]")
    m.stabilizing_column_fixed_cost = Param(initialize = 1.126 ,doc = "stabilizing column fixed cost [$1e3 per year]")
    m.stabilizing_column_linear_coeffcient = Param(initialize = 0.375 ,doc = "stabilizing column linear coeffcient (times number of trays) [$1e3 per year]")
    m.benzene_column_fixed_cost = Param(initialize = 16.3 ,doc = "benzene column fixed cost [$1e3 per year]")
    m.benzene_column_linear_coeffcient =  Param(initialize = 1.55 ,doc = "benzene column linear coeffcient (times number of trays) [$1e3 per year]")
    m.toluene_column_fixed_cost = Param(initialize = 3.9 ,doc = "toluene column fixed cost [$1e3 per year]")
    m.toluene_column_linear_coeffcient =  Param(initialize = 1.12 ,doc = "toluene column linear coeffcient (times number of trays) [$1e3 per year]")
    m.furnace_fixed_cost = Param(initialize = 6.20 ,doc = "toluene column fixed cost [$1e3 per year]")
    m.furnace_linear_coeffcient =  Param(initialize = 1171.7 ,doc = "furnace column linear coeffcient [1e9kj/yr] [$1e3 per year]")
    m.membrane_seperator_fixed_cost = Param(initialize = 43.24 ,doc = "membrane seperator fixed cost [$1e3 per year]")
    m.membrane_seperator_linear_coeffcient =  Param(initialize = 49.0 ,doc = "furnace column linear coeffcient (times inlet flowrate) [$1e3 per year]")
    m.adiabtic_reactor_fixed_cost = Param(initialize = 74.3 ,doc = "adiabatic reactor fixed cost [$1e3 per year]")
    m.adiabtic_reactor_linear_coeffcient = Param(initialize = 1.257 ,doc = "adiabatic reactor linear coeffcient (times reactor volumn) [$1e3 per year]")
    m.isothermal_reactor_fixed_cost = Param(initialize = 92.875 ,doc = "isothermal reactor fixed cost [$1e3 per year]")
    m.isothermal_reactor_linear_coeffcient = Param(initialize = 1.57125 ,doc = "isothermal reactor linear coeffcient (times reactor volumn) [$1e3 per year]")
    m.h2_feed_cost = Param(initialize = 2.5, doc = "h2 feed cost (95% h2,5% Ch4)")
    m.toluene_feed_cost = Param(initialize = 14., doc = "toluene feed cost (100% toluene)")
    m.benzene_product = Param(initialize = 19.9,doc = "benzene product profit(benzene >= 99.97%)")
                    ...
                    for Methane_product_selection in Methane_product_selections:
                        for Toluene_product_selection in Toluene_product_selections:
                            ...
                            res = opt.solve(m,tee =False,
                                            ...
                                            nlp_solver = 'gams',
                                            nlp_solver_args= dict(solver = 'ipopth', add_options = ['option optcr = 0'],warmstart=True),
                                            minlp_solver = 'gams',
                                            minlp_solver_args= dict(solver = 'dicopt', warmstart=True),
                                            subproblem_presolve=False,
                                            init_strategy = 'no_init',
