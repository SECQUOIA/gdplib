...
def HDA_model():
    ...
    m.disteff = Param(initialize=0.5000, doc="column tray effiency")
    ...
    for abso in m.abs:
        for compon in m.compon:
            m.beta[abso, compon].setlb(log(
                (1 - m.aabs[compon] ** (m.nabs[1].lb * m.abseff + m.eps1 + 1)) / (1 - m.aabs[compon])))
            m.beta[abso, compon].setub(min(15, log(
                (1 - m.aabs[compon] ** (m.nabs[1].ub * m.abseff + m.eps1 + 1)) / (1 - m.aabs[compon]))))
    m.t[67].setlb(3.0)
    m.t[67].setub(3.0)
    for compon in m.compon:
    ...
    m.actual_reflux_ratio = Param(
        initialize=1.2, doc="actual reflux ratio coeffieicnt")
    ...
    def build_distillation(b, dist):
        ...
        b.relvol = Constraint([dist], rule=Relvol,
                              doc='average relative volatilty')
        ...
        def Distspec(_m, dist_, stream, compon):
            if (dist_, stream) in m.vdist and (dist_, compon) in m.dhkey and dist_ == dist:
                return m.fc[stream, compon] <= m.recovery_specification_coeffieicnt * sum(m.fc[str2, compon] for (dist_, str2) in m.idist if dist == dist_)
            return Constraint.Skip
    ...
    def build_pump(b, pump_):
        ...
        b.pumpcmb = Constraint(
            [pump_], m.compon, rule=Pumpcmb, doc='component balance')
    ...
    def build_multiple_splitter(b, multi_splitter):
        ...
        b.splpi = Constraint([multi_splitter], m.str,
                             rule=Splpi, doc='inlet pressure relation')
        ...
        b.splpo = Constraint([multi_splitter], m.str,
                             rule=Splpo, doc='outlet pressure relation')
        ...
        b.splti = Constraint([multi_splitter], m.str,
                             rule=Splti, doc='inlet temperature relation')
    ...
    def build_single_splitter(b, splitter):
        ...
        b.spl1ti = Constraint([splitter], m.str, rule=Spl1ti,
                              doc='inlet temperature relation')
        ...
        b.spl1to = Constraint([splitter], m.str, rule=Spl1to,
                              doc='outlet temperature relation')
    ...
    m.adiabtic_reactor_fixed_cost = Param(initialize = 74.3 ,doc = "adiabtic reactor fixed cost ($1e3 per year)")
    m.adiabtic_reactor_linear_coeffcient = Param(initialize = 1.257 ,doc = "adiabtic reactor linear coeffcient (times reactor volumn) ($1e3 per year)")
