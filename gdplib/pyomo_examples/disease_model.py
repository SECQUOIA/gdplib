...
def build_model():
    ...
    def _beta_bounds(m):
        ...
        Returns:
            None, but set up the transition parameters bounds. 
        """
        return (None,5.0)
    ...
    def _obj_rule(m):
        """
        Minimize the total discrepancy or error in the SIR disease model model over time.
        ...
        Returns:
            expr (Pyomo.Expression): The objective function expression, which is the sum of the variables 'p' and 'n' for each time period. 
                This expression represents the total error (both overestimation and underestimation) across all time periods in the model.
        """
    ...
    def _logSIR(m,i):
        ...
        return (0.0, expr)
    model.logSIR = Constraint(model.S_meas_small, rule=_logSIR, doc='log of SIR disease model')

    # objective function constraint
    def _p_n_const(m,i):
        """
        Defines a constraint for the SIR disease model that relates the model's estimated infectious cases to observed data.
        This constraint is essential for ensuring the model's estimates are consistent with actual data, adjusted for overestimation and underestimation.
        ...
        Returns:
            A tuple (0.0, expr) where expr is the Pyomo expression for the constraint at the i-th biweekly period.
            This represents the adjustment of the model's estimated cases by accounting for overestimation and underestimation errors compared to the observed data.
        """
        expr = logIstar[i-1] - m.logI[i] - m.p[i] + m.n[i]
        return (0.0, expr)
    model.p_n_const = Constraint(model.S_meas,rule=_p_n_const, doc='constraint for p and n')
    ...
    def _high_low(disjunct, i, y):
        """
        Disjunct for the high and low beta values

        Args:
            disjunct (Pyomo.Disjunction): _description_
            i (int): index of biweekly periods in the data set
            y (int): binary variable

        Returns:
            None, but sets up the disjuncts for the high and low beta values.
        """
    ...
    model.high_low = Disjunct(model.S_beta, model.y, rule=_high_low, doc='disjunct for high and low beta values')
    ...
    def _disj(model, i):
        ...
        Args:
            model (Pyomo.Disjunction): disjunction for the high and low beta values
            i (int): index of biweekly periods in the data set

        Returns:
            A list of disjuncts (model.high_low[i, j]) for each binary state 'j' in 'model.y'. 
            This list represents the two possible states for each biweekly period 'i':
                - model.high_low[i, 0]: The set of constraints when the low beta value is active.
                - model.high_low[i, 1]: The set of constraints when the high beta value is active.
    ...
    model.disj = Disjunction(model.S_beta, rule=_disj, doc='disjunction for high and low beta values')
