...
def build_model():
    ...
    m.base_tech_capital_cost = Var(
        m.syngas_techs, m.syngas_tech_units, bounds=(0, None)
    )
    ...
    m.syngas_tech_emissions = Var(
        bounds=(0, None), doc="CO2 emission of syngas processes"
    )
    ...
    m.syngas_total_flow = Expression(
        expr=sum(m.final_syngas_flow[species] for species in {'H2', 'CO2', 'CO', 'CH4'})
    )
